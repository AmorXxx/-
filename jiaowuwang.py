...
for i in range(2016026358, 2016026399):
    ...
    if browser.find_elements_by_class_name('errorTop'):
        ...
        try:
            ...
            dataCount = int(mysqlCommand.getLastId_s()) + 1
        except Exception:
            browser.switch_to.frame(browser.find_element_by_xpath("//frame[@src='/menu/t_top.jsp']"))
            st = '教师'
            dataCount = int(mysqlCommand.getLastId_t()) + 1
    ...
    try:
        if st == '学生':
            ...
            # 插入数据，如果已经存在就不在重复插入
            res = mysqlCommand.insertData_s(news_dict)
        else:

            res = mysqlCommand.insertData_t(news_dict)
        if res:
            dataCount = res
